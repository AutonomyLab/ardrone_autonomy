...
main(int argc, char **argv)
{
  ...
  if( codec < 0 )
  {
    PRINT("Usage : %s [encode|decode] <input file name> <output file name>.\n", argv[0]);
    return EXIT_FAILURE;
  }
...
PROTO_THREAD_ROUTINE(app,argv)
{
  ...
  ifc.name                    = ((char**)argv)[2];
  ifc.buffer_size             = (ACQ_WIDTH*ACQ_HEIGHT*3)/2;

  ofc.name                    = ((char**)argv)[3];
...
bool_t dct_compute( dct_io_buffer_t* io_buffer )
{
  ...
  {
//    if( vp_os_mutex_trylock(&critical_section) )
    {
      current_io_buffer = io_buffer;
/*
      vp_os_mutex_lock(&dct_start_mutex);
        vp_os_cond_signal(&dct_start_cond);
      vp_os_mutex_unlock(&dct_start_mutex);
*/
      res = TRUE;
...
/*
  if( result_io_buffer != NULL )
  {
    if( vp_os_mutex_trylock(&critical_section) )
    {
      io_buffer = result_io_buffer;
      result_io_buffer = NULL;
